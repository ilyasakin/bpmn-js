...
// Write screenshots to a temp dir by default so they don't litter the
// repo root. Override via `OUT_DIR=/some/path` env var.
...
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" id="D" targetNamespace="x">
  ...
  await new Promise(r => setTimeout(r, 200));
  await page.screenshot({ path: path.join(outDir, 'editor-task.png') });
  ...
  await new Promise(r => setTimeout(r, 200));
  await page.screenshot({ path: path.join(outDir, 'editor-event.png') });
  ...
  await new Promise(r => setTimeout(r, 200));
  await page.screenshot({ path: path.join(outDir, 'editor-edge.png') });
