...
/**
 * Compute simple two-point waypoints from source mid to target mid.
 * Good enough for v1; replaced by a proper layouter later.
 */
...
export default function BpmnXyflowModeler(options = {}) {
  ...
  function rerouteEdge(edge) {
    edge.waypoints = computeWaypoints(edge.source, edge.target);
    if (edge.di && edge.di.waypoint) {
      // sync DI waypoints
  ...
  function setNodePosition(node, x, y) {
    ...
    // edges connected to this node need rerouting
    getEdgesConnectedTo(node).forEach(rerouteEdge);
  ...
  function onPointerDown(evt) {
    ...
    if (connectState) return; // connect mode handled elsewhere

    const id = internals.findElementId(evt.target);
    const node = id ? viewer.getElement(id) : null;
    if (!node || node.type === 'label' || !('x' in node)) return;
    if (evt.shiftKey) return; // reserve shift+drag for connect

    const start = internals.toGraph(evt.clientX, evt.clientY);
    dragState = {
      node,
      origin: { x: node.x, y: node.y },
      offset: { x: start.x - node.x, y: start.y - node.y },
      moved: false
    };

    evt.stopPropagation(); // prevent panZoom from grabbing the gesture
  }
  ...
  function attachModeling() {
    ...
    svg.addEventListener('pointerdown', onShapeDownForConnect, true);
  ...
  function detachModeling() {
    ...
    attached = false;
    const svg = viewer.getSvg();
    svg.removeEventListener('pointerdown', onShapeDownForConnect, true);
  ...
  function onShapeDownForConnect(evt) {
  ...
  function onPointerMoveCapture(evt) {
    if (connectState) {
      const p = internals.toGraph(evt.clientX, evt.clientY);
  ...
  function onPointerUpCapture(evt) {
    if (connectState) {
      const id = internals.findElementId(evt.target);
      const node = id ? viewer.getElement(id) : null;
