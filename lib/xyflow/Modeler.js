...
export default function BpmnXyflowModeler(options = {}) {
  ...
  function updateLabelNodeDrag(evt) {
    ...
    n.x = newX; n.y = newY;
    if (n.di && n.di.bounds) {
      n.di.bounds.x = newX; n.di.bounds.y = newY;
    }
  ...
  function endLabelNodeDrag() {
    ...
    // persist via host's di.label.bounds so round-trip is stable
    const host = node.labelTarget;
    const hostDi = host && host.di;
    const oldHostLabelBounds = hostDi && hostDi.label && hostDi.label.bounds
      ? { x: hostDi.label.bounds.x, y: hostDi.label.bounds.y, width: hostDi.label.bounds.width, height: hostDi.label.bounds.height }
      : null;

    commands.execute({
      name: 'move-external-label',
      do: () => {
        node.x = final.x; node.y = final.y;
        if (node.di && node.di.bounds) { node.di.bounds.x = final.x; node.di.bounds.y = final.y; }
        if (hostDi) {
          if (!hostDi.label) {
            hostDi.label = moddle.create('bpmndi:BPMNLabel', {
              bounds: moddle.create('dc:Bounds', { x: final.x, y: final.y, width: node.width, height: node.height })
            });
          } else {
            hostDi.label.bounds = moddle.create('dc:Bounds', { x: final.x, y: final.y, width: node.width, height: node.height });
          }
        }
        const g = internals.elementGfx(node.id);
        if (g) g.setAttribute('transform', `translate(${ final.x }, ${ final.y })`);
      },
      undo: () => {
        node.x = origin.x; node.y = origin.y;
        if (node.di && node.di.bounds) { node.di.bounds.x = origin.x; node.di.bounds.y = origin.y; }
        if (hostDi) {
          if (oldHostLabelBounds) {
            hostDi.label.bounds = moddle.create('dc:Bounds', oldHostLabelBounds);
          } else {
            hostDi.label = null;
          }
        }
        const g = internals.elementGfx(node.id);
        if (g) g.setAttribute('transform', `translate(${ origin.x }, ${ origin.y })`);
      }
    });
  }
  ...
  function endResize() {
    if (!resizeDragState) return;
    const { node, origin, anchors, moved, containerChildren, childOrigins, attachers, attacherOrigins } = resizeDragState;
    ...
    const childFinals = (containerChildren || []).map(c => ({
      node: c, x: c.x, y: c.y,
      edges: getEdgesConnectedTo(c).map(e => ({ edge: e, wp: e.waypoints.map(p => ({ ...p })) }))
    }));
    const attacherFinals = (attachers || []).map(a => ({
      node: a, x: a.x, y: a.y,
      edges: getEdgesConnectedTo(a).map(e => ({ edge: e, wp: e.waypoints.map(p => ({ ...p })) }))
    }));
    ...
    commands.execute({
      ...
      do: () => {
        ...
        childFinals.forEach(cf => {
          cf.node.x = cf.x; cf.node.y = cf.y;
          if (cf.node.di && cf.node.di.bounds) {
            cf.node.di.bounds.x = cf.x; cf.node.di.bounds.y = cf.y;
          }
          const g = internals.elementGfx(cf.node.id);
          if (g) g.setAttribute('transform', `translate(${ cf.x }, ${ cf.y })`);
          cf.edges.forEach(({ edge, wp }) => {
            edge.waypoints = wp.map(p => ({ ...p }));
            syncEdgeDi(edge);
            internals.redrawConnection(edge);
          });
        });
        attacherFinals.forEach(af => {
          af.node.x = af.x; af.node.y = af.y;
          if (af.node.di && af.node.di.bounds) {
            af.node.di.bounds.x = af.x; af.node.di.bounds.y = af.y;
          }
          const g = internals.elementGfx(af.node.id);
          if (g) g.setAttribute('transform', `translate(${ af.x }, ${ af.y })`);
          af.edges.forEach(({ edge, wp }) => {
            edge.waypoints = wp.map(p => ({ ...p }));
            syncEdgeDi(edge);
            internals.redrawConnection(edge);
          });
      ...
      undo: () => {
        ...
        (childOrigins || []).forEach(co => {
          co.node.x = co.x; co.node.y = co.y;
          if (co.node.di && co.node.di.bounds) {
            co.node.di.bounds.x = co.x; co.node.di.bounds.y = co.y;
          }
          const g = internals.elementGfx(co.node.id);
          if (g) g.setAttribute('transform', `translate(${ co.x }, ${ co.y })`);
          co.edges.forEach(({ edge, wp }) => {
            edge.waypoints = wp.map(p => ({ ...p }));
            syncEdgeDi(edge);
            internals.redrawConnection(edge);
          });
        });
        (attacherOrigins || []).forEach(ao => {
          ao.node.x = ao.origin.x; ao.node.y = ao.origin.y;
          if (ao.node.di && ao.node.di.bounds) {
            ao.node.di.bounds.x = ao.origin.x; ao.node.di.bounds.y = ao.origin.y;
          }
          const g = internals.elementGfx(ao.node.id);
          if (g) g.setAttribute('transform', `translate(${ ao.origin.x }, ${ ao.origin.y })`);
          ao.edges.forEach(({ edge, wp }) => {
            edge.waypoints = wp.map(p => ({ ...p }));
            syncEdgeDi(edge);
            internals.redrawConnection(edge);
          });
  ...
  function deleteElement(element) {
    ...
    const containerKids = isContainer(element)
      ? (element.children || []).filter(c => c !== element).slice()
      : [];
  ...
  function openLabelEditor(node, evt) {
    ...
    Object.assign(editor.style, {
      ...
      zIndex: 11,
      outline: 'none',
      // vertical centring for inline-label shapes
      display: 'flex',
      alignItems: 'center',
      justifyContent: region.textAlign === 'center' ? 'center' : 'flex-start',
      // single inner div so flex centres correctly while contenteditable
  ...
  function closeLabelEditor(commit) {
    ...
    if (commit) labelEditor.blur();
      ...
      // trigger our own cancel — blur would commit
      const evt = new KeyboardEvent('keydown', { key: 'Escape', cancelable: true });
      labelEditor.dispatchEvent(evt);
    }
    labelEditor = null;
  }
