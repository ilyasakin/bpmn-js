...
export default function BpmnXyflowModeler(options = {}) {
  ...
  function updateResize(evt) {
    ...
    applyResize(node, nx, ny, nw, nh);
    resizeDragState.moved = true;
  }

  function applyResize(node, x, y, w, h) {
    // capture the old bounds so we can re-anchor docked endpoints at
    // the same relative position before re-cropping
    const oldBounds = { x: node.x, y: node.y, w: node.width, h: node.height };

    // capture each connected edge's docked-endpoint relative position
    const edges = getEdgesConnectedTo(node);
    const relAnchors = edges.map(edge => {
      const sourceIs = edge.source === node;
      const targetIs = edge.target === node;
      const sIdx = 0;
      const tIdx = edge.waypoints.length - 1;
      const rel = (wp) => ({
        rx: oldBounds.w === 0 ? 0 : (wp.x - oldBounds.x) / oldBounds.w,
        ry: oldBounds.h === 0 ? 0 : (wp.y - oldBounds.y) / oldBounds.h
      });
      return {
        edge,
        source: sourceIs ? rel(edge.waypoints[sIdx]) : null,
        target: targetIs ? rel(edge.waypoints[tIdx]) : null
      };
    });
    ...
    // re-anchor each docked endpoint to the same relative position on
    // the new bounds, then re-crop against the new shape outline so it
    // sits on the actual boundary (circle, diamond, rounded rect, …)
    const bpmnRenderer = internals.renderer && internals.renderer.bpmnRenderer;
    relAnchors.forEach(({ edge, source, target }) => {
      if (source) {
        edge.waypoints[0] = { x: x + source.rx * w, y: y + source.ry * h };
      }
      if (target) {
        const i = edge.waypoints.length - 1;
        edge.waypoints[i] = { x: x + target.rx * w, y: y + target.ry * h };
      }
      if (bpmnRenderer) {
  ...
  function endResize() {
    if (!resizeDragState) return;
    const { node, origin, moved } = resizeDragState;
    ...
    commands.execute({
      name: 'resize',
      do: () => applyResize(node, final.x, final.y, final.w, final.h),
      undo: () => {
        applyResize(node, origin.x, origin.y, origin.w, origin.h);
