...
export default function BpmnXyflowModeler(options = {}) {
  ...
  function onMouseDown(evt) {
    ...
    draggedNodes.forEach(n => getEdgesConnectedTo(n).forEach(e => edgeSet.add(e)));
  ...
  function applyReparent({ node, newParent }) {
    ...
    // moddle-side: move flowElement into the new container's process if it
    // owns flowElements (Process / SubProcess); otherwise leave the
    // moddle parent — Lanes/Participants don't directly own flowElements,
    // they reference them via flowNodeRef.
    ...
    if (newOwnerBo && newOwnerBo.$instanceOf && newOwnerBo.$instanceOf('bpmn:Lane')) {
      // lane → use its parent process; track laneNodeRef
      let p = newOwnerBo.$parent;
      while (p && !p.$instanceOf('bpmn:Process')) p = p.$parent;
      // wire flowNodeRef on the lane
  ...
  function ownerForConnection(connType, source, target) {
    ...
    // SequenceFlow / Association / DataAssociation owned by source's process
    let p = source.businessObject.$parent;
    while (p && !instanceOfBpmnProcess(p)) p = p.$parent;
    return p || findOwningProcess();
  }

  function instanceOfBpmnProcess(bo) {
    return bo && typeof bo.$instanceOf === 'function' && bo.$instanceOf('bpmn:Process');
  }
  ...
  function createConnection(source, target, options = {}) {
    ...
    commands.execute({
      ...
      do: () => {
        ...
        if (connType === 'bpmn:SequenceFlow') {
          if (Array.isArray(source.businessObject.outgoing) && !source.businessObject.outgoing.includes(businessObject)) source.businessObject.outgoing.push(businessObject);
          if (Array.isArray(target.businessObject.incoming) && !target.businessObject.incoming.includes(businessObject)) target.businessObject.incoming.push(businessObject);
        }
        internals.redrawConnection(edge);
      },
      undo: () => {
  ...
  function toggleSubProcessExpanded(node) {
    ...
    commands.execute({
      ...
      do: () => { node.di.isExpanded = after; node.collapsed = after === false; internals.redrawShape(node); },
      undo: () => { node.di.isExpanded = before; node.collapsed = before === false; internals.redrawShape(node); }
    });
  }

  // Maintain a navigation stack so we can pop back from a drill-in.
  const navStack = [];

  function drillInto(subProcessNode) {
    if (!subProcessNode || !subProcessNode.businessObject) return;
    const defs = viewer.getDefinitions();
    if (!defs) return;
    const bo = subProcessNode.businessObject;
    // Find a diagram whose plane references this sub-process; otherwise
    // synthesise one in-memory and add it to defs.diagrams.
    ...
    navStack.push(viewer.getDefinitions());
    // Re-import using the synthesised diagram. The viewer's importXML
    // path expects an XML string, so we serialize+reimport.
    serializeAndReimport(diagram.id);
  }

  function navigateBack() {
    if (!navStack.length) return;
    const prev = navStack.pop();
    void prev; // we re-serialize and re-import the root diagram
    serializeAndReimport(null);
  }

  async function serializeAndReimport(diagramId) {
    const xml = await self.getXML();
    await self.importXML(xml, diagramId || undefined);
  }
