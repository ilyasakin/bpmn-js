...
export default function BpmnXyflowViewer(options = {}) {
  ...
  this._internals = {
    ...
    redrawConnection(edge) {
      ...
      try { renderer.drawConnection(g, edge); } catch (e) { console.warn('redrawConnection', edge.id, e); }

      // Render connection name label at the longest segment's midpoint.
      // Users can drag the label later (handled by the modeler).
      const name = edge.businessObject && edge.businessObject.name;
      if (name && edge.waypoints && edge.waypoints.length >= 2) {
        const labelPos = edge.labelOffset || pickLabelPosition(edge.waypoints);
        const labelG = svgCreate('g', {
          'class': 'bpmn-xyflow-connection-label',
          'data-element-id': edge.id + '__label',
