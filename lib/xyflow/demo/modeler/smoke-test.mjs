...
try {
  ...
  // 3. drag the task: simulate pointer move and verify waypoints rerouted
  const moveResult = await page.evaluate(async () => {
    ...
    taskGfx.dispatchEvent(new PointerEvent('pointerdown', downOpts));
    await new Promise(r => requestAnimationFrame(r));
    window.dispatchEvent(new PointerEvent('pointermove', { bubbles: true, clientX: startX + 200, clientY: startY + 80 }));
    await new Promise(r => requestAnimationFrame(r));
    window.dispatchEvent(new PointerEvent('pointerup', { bubbles: true, clientX: startX + 200, clientY: startY + 80 }));
    await new Promise(r => requestAnimationFrame(r));

    const moved = m.getElement(task.id);
    const edge = m.getGraph().edges[0];
    const expectedTargetMid = { x: moved.x + moved.width / 2, y: moved.y + moved.height / 2 };
    const lastWp = edge.waypoints[edge.waypoints.length - 1];
    return {
      ok: Math.abs(lastWp.x - expectedTargetMid.x) < 1 && Math.abs(lastWp.y - expectedTargetMid.y) < 1,
      moved: { x: moved.x, y: moved.y },
      lastWp,
      expected: expectedTargetMid
    };
  });
  console.log(`${ moveResult.ok ? 'OK ' : 'FAIL' }  drag-move reroutes edge  (target=(${ moveResult.expected.x },${ moveResult.expected.y }) wp=(${ moveResult.lastWp.x },${ moveResult.lastWp.y }))`);
